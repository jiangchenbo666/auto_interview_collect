...
def test_has_llm_config_returns_false_without_key(monkeypatch):
    ...
    assert provider.has_llm_config("kimi") is False
    assert provider.has_llm_config("moonshot") is False
    assert provider.has_llm_config("openai") is False


def test_has_llm_config_accepts_kimi_key(monkeypatch):
    monkeypatch.setattr(provider, "load_env_file", lambda: None)
    monkeypatch.setenv("KIMI_API_KEY", "fake-key")

    assert provider.has_llm_config("kimi") is True
